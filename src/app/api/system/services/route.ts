...
// Service action allowlists are environment-driven in the public build.
// Fall back to the matching watched-service env vars for backward compatibility.
const ALLOWED_SERVICES_PM2 = parseServiceList(
  process.env.PM2_ALLOWED_SERVICES || process.env.PM2_WATCHED_SERVICES
);
const ALLOWED_SERVICES_SYSTEMD = parseServiceList(
  process.env.SYSTEMD_ALLOWED_SERVICES || process.env.SYSTEMD_WATCHED_SERVICES
);
...
async function pm2Action(name: string, action: string): Promise<string> {
  ...
  if (action === 'logs') {
    ...
    try {
      const pm2LogDir = process.env.PM2_LOG_DIR || `${process.env.HOME || '/root'}/.pm2/logs`;
      const logFile = `${pm2LogDir}/${name}-out.log`;
      const { stdout } = await execAsync(`tail -100 "${logFile}" 2>/dev/null || echo "No logs available"`);
      const errFile = `${pm2LogDir}/${name}-error.log`;
