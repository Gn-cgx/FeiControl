...
export function calculateSnapshot(
  ...
  for (const [key, group] of grouped.entries()) {
    ...
    const cacheReadTokens = group.reduce((sum, s) => sum + s.cacheReadTokens, 0);
    const cacheWriteTokens = group.reduce((sum, s) => sum + s.cacheWriteTokens, 0);
...
export function saveSnapshot(
  ...
  const stmt = db.prepare(`
    INSERT INTO usage_snapshots 
      (timestamp, date, hour, agent_id, model, input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, total_tokens, cost)
    VALUES 
      (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
